...
class httpCheck(  ):
    ...
    redisObj = None
    logObj = None
    alertTimeOutUpper2s=0.10
    alertTimeOutUpper1s=0.10
    alertFailedRate=0.10
    warningTimeOutUpper2s=0.05
    warningTimeOutUpper1s=0.05
    warningFailedRate=0.05
    nscaTmpl=""
    stopFlag=False
    conf=None
    retMsgIP=""
    ...
    def __init__( self,  _logObj=None,_redisObj=None, _conf=None ):
        ...
        self.alertTimeOutUpper1s = globalVariable.threshold["httpResponseTimeAreaUpper1s"]
        self.alertTimeOutUpper2s = globalVariable.threshold["httpResponseTimeAreaUpper2s"]
        self.warningFailedRate = globalVariable.threshold["httpStatusCodeFailedRate_warning"]
        self.warningTimeOutUpper1s = globalVariable.threshold["httpResponseTimeAreaUpper1s_warning"]
        self.warningTimeOutUpper2s = globalVariable.threshold["httpResponseTimeAreaUpper2s_warning"]
    ...
    def run(self):
        ...
        while True:
            ...
            self.logObj.info('brpop queue(%s) and blocktime(%s)'%(self.conf['queue']['queuekeynamepro'], self.conf['queue']['blocktimerout']))
            queueNodeTuplePro=self.redisObj.brpop(self.conf['queue']['queuekeynamepro'], int(self.conf['queue']['blocktimerout']))
	        #hostApiList = list(self.redisObj.smembers('apiKeyList')) 
            self.stopFlag=False
            if queueNodeTuple or queueNodeTuplePro:
    ...
    def mergeIPData(self,_ip):
        ...
        if (sumSuccessHit < self.everyM and sumTotalHitKey<self.everyM):
            ...
            if ( rate1 > self.alertFailedRate or rate2 > self.alertTimeOutUpper1s or rate3 > self.alertTimeOutUpper2s ):
                _status = "CRITICAL"
            elif ( rate1 > self.warningFailedRate or rate2 > self.warningTimeOutUpper1s or rate3 > self.warningTimeOutUpper2s ):
                _status = "WARNING"
            else:
                _status = "OK"
            if sumTotalHitKey==0:
        ...
        retMsgIP = "HTTP_CHECK IP %s:5xx(5xx:%d/total:%d)=%.2f%%;upper1s((2xx:%d-<500:%d-<1s:%d)/2xx:%d)=%.2f%%;upper2s((2xx:%d-<500:%d-<1s:%d-<2s:%d)/2xx:%d)=%.2f%%|avgTime=%.2fs"%(_status,sumTotal5xx,sumTotalHitKey,rate1*100,sumSuccessHit,sumSuccessLess500,sumSuccessLess1s,sumSuccessHit,rate2*100,sumSuccessHit,sumSuccessLess500,sumSuccessLess1s,sumSuccessLess2s,sumSuccessHit,rate3*100,avgTime)
    ...
    def mergeIDCData(self,idcName):
        ...
        retMsgIDC = "HTTP_CHECK IDC %s:5xx(5xx:%d/total:%d)=%.2f%%;upper1s((2xx:%d-<500:%d-<1s:%d)/2xx:%d)=%.2f%%;upper2s((2xx:%d-<500:%d-<1s:%d-<2s:%d)/2xx:%d)=%.2f%%|avgTime=%.2fs"%(_status,sumTotal5xx,sumTotalHitKey,rate1*100,sumSuccessHit,sumSuccessLess500,sumSuccessLess1s,sumSuccessHit,rate2*100,sumSuccessHit,sumSuccessLess500,sumSuccessLess1s,sumSuccessLess2s,sumSuccessHit,rate3*100,avgTime)
    ...
    def check( self, _ip, _interface ):
        ...
        retMsg = "HTTP_CHECK API %s:5xx(5xx:%d/total:%d)=%.2f%%;upper1s((2xx:%d-<500:%d-<1s:%d)/2xx:%d)=%.2f%%;upper2s((2xx:%d-<500:%d-<1s:%d-<2s:%d)/2xx:%d)=%.2f%%|avgTime=%.2fs"%(_status,returnResult[total5xxKey],returnResult[totalHitsKey2],rate1*100,returnResult[successHitsKey1],returnResult[successLessKey1],returnResult[successLessKey2],returnResult[successHitsKey1],rate2*100,returnResult[successHitsKey1],returnResult[successLessKey1],returnResult[successLessKey2],returnResult[successLessKey3],returnResult[successHitsKey1],rate3*100,avgTime)
